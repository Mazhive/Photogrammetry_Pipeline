...
class PhotogrammetryApp(QMainWindow):
    ...
    def run_full_pipeline(self):
        ...
        steps = [
            ...
            ("InterfaceCOLMAP", f"docker run --rm --user {user_id}:{group_id} -v '{self.base_dir}':/pipeline {engine} bash -c 'cd /pipeline/projects/{name} && /pipeline/bin/InterfaceCOLMAP -i colmap_data -o model.mvs'", None),
            ("DensifyPointCloud", f"docker run --rm --user {user_id}:{group_id} -v '{self.base_dir}':/pipeline {engine} bash -c 'cd /pipeline/projects/{name} && /pipeline/bin/DensifyPointCloud model.mvs'", None),
            ("ReconstructMesh", f"docker run --rm --user {user_id}:{group_id} -v '{self.base_dir}':/pipeline {engine} bash -c 'cd /pipeline/projects/{name} && /pipeline/bin/ReconstructMesh model_dense.mvs'", None),
            ("TextureMesh", f"docker run --rm --user {user_id}:{group_id} -v '{self.base_dir}':/pipeline {engine} bash -c 'cd /pipeline/projects/{name} && /pipeline/bin/TextureMesh model_dense.mvs --mesh-file model_dense_mesh.ply --export-type obj -o model_final.obj'", None)
        ]
